...
- [API Documentation](#api-documentation)
  - [Synchronous API (`Result`)](#synchronous-api-result)
    ...
    - [`Result.and` (method)](#resultand-method)
    - [`Result.or` (method)](#resultor-method)
...
Returns a default value wrapped in an `Ok` if the result is an `Ok` variant.
Otherwise, returns the `Err` value untouched.
...
```typescript
import { centerMapOnLocation } from './imaginary-map-utils'

const defaultMapLocation = { lat: 0, lng: 0 }
const countryLocations = [
  { name: 'Japan', location: { lat: 45.5, lng: -73.5 } },
  { name: 'France', location: { lat: 37.5, lng: -122.5 } },
  { name: 'Congo', location: { lat: 19.5, lng: -99.5 } },
]
...
const addCountry = (newCountry: {
  ...
  if (countries.find((country) => country.name === newCountry.name)) {
    return err('Country already exists')
  }
  countries.push(newCountry)
  return ok()
}

const newCountry1 = { name: 'New Zealand', location: { lat: 12, lng: 42 } }
addCountry(newCountry).and(newCountry1.name) // Ok('New Zealand')

const newCountry2 = { name: 'Japan', location: { lat: 56, lng: 55 } }
addCountry(newCountry).and(newCountry2.name) // Err('Country already exists')
```
...
```typescript
class Result<T, E> {
  or(defaultValue: A): Result<T | A, E> { ... }
}
```
...
const getCountryLocation = (countryName: string): Result<Location, Error> => {
  const country = countries.find((country) => country.name === countryName)
  return country ? ok(country.location) : err('Country not found')
}

const currentCountry = 'New Zealand'

// Since the country can't be found, the default location is used
getCountryLoation(currentCountry).or(defaultMapLocation).andTee(centerMapOnLocation)
```
